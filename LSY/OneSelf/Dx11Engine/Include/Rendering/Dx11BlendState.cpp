	...
	BOOL bIndependentEnable)
{
	...
	if (FAILED(DX11_DEVICE->CreateBlendState(&m_tDesc, (ID3D11BlendState**)&m_pState)))
		return false;

	return true;
}

bool CDx11BlendState::CreateBlendState(const D3D11_RENDER_TARGET_BLEND_DESC * pTargetBlend, UINT iCount, BOOL bAlphaEnable, BOOL bIndependentEnable)
{
	m_tDesc.AlphaToCoverageEnable = bAlphaEnable;
	m_tDesc.IndependentBlendEnable = bIndependentEnable;
...
{
	DX11_CONTEXT->OMGetBlendState((ID3D11BlendState**)&m_pPrevState, m_fOldBlendFactor,
		&m_iOldSampleMask);
	DX11_CONTEXT->OMSetBlendState((ID3D11BlendState*)m_pState, m_fBlendFactor,
		0xffffffff);
}

void CDx11BlendState::ResetState()
{
	DX11_CONTEXT->OMSetBlendState((ID3D11BlendState*)m_pPrevState, m_fOldBlendFactor,
		m_iOldSampleMask);
