...
CDx11GameObject::CDx11GameObject(const CDx11GameObject & obj)
{
	*this = obj;
	...
	list<CDx11GameObject*>::const_iterator	iter2End = obj.m_MonsterList.end();

	for (iter2 = obj.m_MonsterList.begin(); iter2 != iter2End; ++iter2)
	{
		CDx11GameObject*	pMonster = (*iter2)->Clone();

		pMonster->SetParent(this);

		m_ChildList.push_back(pMonster);
	}
}
...
void CDx11GameObject::SetScene(CDx11Scene * pScene)
{
	...
	list<CDx11GameObject*>::const_iterator	iter2;
	list<CDx11GameObject*>::const_iterator	iter2End = m_MonsterList.end();

	for (iter2 = m_MonsterList.begin(); iter2 != iter2End; ++iter2)
	{
		(*iter2)->SetScene(pScene);
	}
}
...
list<CDx11GameObject*>* CDx11GameObject::GetMonsterList()
{
...
void CDx11GameObject::Input(float fTime)
{
	...
	list<CDx11GameObject*>::iterator	iter2End = m_MonsterList.end();

	for (iter2 = m_MonsterList.begin(); iter2 != iter2End;)
	{
		if (!(*iter2)->GetEnable())
		{
			++iter2;
...
CDx11GameObject * CDx11GameObject::FindMonsterFromTag(const string & strTag)
{
	list<CDx11GameObject*>::iterator	iter;
	list<CDx11GameObject*>::iterator	iterEnd = m_MonsterList.end();

	for (iter = m_MonsterList.begin(); iter != iterEnd; ++iter)
	{
		if ((*iter)->GetTag() == strTag)
		{
			(*iter)->AddRef();
...
CDx11GameObject * CDx11GameObject::FindMonsterFromTypeName(const string & strTypeName)
{
	list<CDx11GameObject*>::iterator	iter;
	list<CDx11GameObject*>::iterator	iterEnd = m_MonsterList.end();
