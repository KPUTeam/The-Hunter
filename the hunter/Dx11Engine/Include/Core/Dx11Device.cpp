...
bool CDx11Device::CreateDepthStencilView()
{
	...
	if (FAILED(m_pDevice->CreateTexture2D(&tDesc, NULL, &m_pDepthTex)))
		return false;

	if (FAILED(m_pDevice->CreateDepthStencilView(m_pDepthTex, 0, &m_pDepthStencilView)))
		return false;
	return true;
}
