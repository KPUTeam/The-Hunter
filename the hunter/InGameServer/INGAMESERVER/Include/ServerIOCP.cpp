...
VOID CServerIOCP::OnIoRead(VOID * object, DWORD dataLength)
{
	...
	if (pSession->ReadPacketForIocp(dataLength))
	{
		...
		//		MonsterPosition(pSession, PacketArr);
				break;
			...
			case PH_USER_WIN:
				UserWin(pSession, PacketArr);
				break;

			case PH_USER_LOSE:
				UserLose(pSession, PacketArr);
				break;
			}
		}
	}
...
void CServerIOCP::UserLogin(CUserSession * pSession, BYTE * pPacket)
{
	...
	{
		pTransform->SetWorldPos(97.f, 0.f, 28.f);
	//	pTransform->SetWorldPos(47.f, 0.f, 95.f);
		cout << "초기 위치 전송" << endl;
	...
	if (CNetworkManager::GetInst()->GetConnectCount() == 3)
	{
		pTransform->SetWorldPos(49.f, 0.f, 96.f);
		cout << "초기 위치 전송" << endl;
		cout << "x: " << pTransform->GetWorldPos().x << ", y: " << pTransform->GetWorldPos().y << ", z: " << pTransform->GetWorldPos().z << endl;
	}
...
void CServerIOCP::MonsterPosition(CUserSession * pSession, BYTE * pPacket)
{
...
//	for (int i = 0; i < 3; ++i)	stream->WriteFloat(vRot[i]);
	...
	CNetworkManager::GetInst()->SendMonsterPosAllUser(pSession, vPos, vRot);
//	pSession->WritePacket(PH_MONSTER_POS, byPacket, stream->GetLength());
//	pSession->WriteComplete();
}
...
void CServerIOCP::UserWin(CUserSession * pSession, BYTE * pPacket)
{
	CZEROStreamSP	stream;
	
	BOOL	bWin = true;

	stream->WriteBOOL(bWin);
	BYTE	byPacket[PACKET_SIZE] = {};
	stream->SetBuffer(byPacket);
	pSession->WritePacket(PH_USER_WIN, byPacket, stream->GetLength());
	pSession->WriteComplete();
}

void CServerIOCP::UserLose(CUserSession * pSession, BYTE * pPacket)
{
	CZEROStreamSP	stream;
	stream->SetBuffer(pPacket);
	CNetworkManager::GetInst()->SendUserResult(pSession);
}
