...
{
	// Bar Frame
	m_pFrameBar = CDx11GameObject::Create("MonsterFrameBar");
	m_pGameObject->AddChild(m_pFrameBar);

	CDx11Renderer*	pRenderer = m_pFrameBar->AddComponent<CDx11Renderer>("MonsterFrameBarRenderer");
	pRenderer->CreateMaterial("MonsterFrameBar", L"Health/HBarFrame.png");
	...
	pTransform->Scaling(2.f, 0.3f, 1.f);
	pTransform->MoveWorld(Vec3( 0, 0, 0));
	SAFE_RELEASE(pTransform);

	// 빌보드 컴포넌트를 추가한다.
	CDx11BillBoard*	pBillBoard = m_pFrameBar->AddComponent<CDx11BillBoard>("MonsterFrameBarBillBoard");
	...
	pRenderer->CreateMaterial("HPBar2", L"Health/HBar.png");
	pRenderer->SetMesh(EFFECT_LT_MESH);
	pRenderer->SetShader(UISHADER);
	pRenderer->SetRenderState(DX11_RS_ALPHABLEND, RSF_BLEND);
	pRenderer->SetRenderState(DX11_RS_DEPTH_DISABLE, RSF_DEPTHSTENCIL);
