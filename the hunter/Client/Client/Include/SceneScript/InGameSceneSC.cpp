...
#include "../ObjScript/Bullet/BulletSC.h"
...
{
	if (!GET_SINGLE(CNetManager)->EmptyLoginUser())
	{
		CDx11Layer* pLayer = m_pScene->FindLayer("DefaultLayer");
		GET_SINGLE(CNetManager)->AddLayer(pLayer);

		m_pUserPlayer = pLayer->FindObjectFromTag("UserObject");
		m_pUserPlayer->AddRef();
		m_pUserTr = m_pUserPlayer->GetTransform();

		m_pBodyObj = m_pUserPlayer->FindChildFromTag("pUserBodyObj");
		m_pHeadObj = m_pBodyObj->FindChildFromTag("pUserHeadObj");
		m_pHeadObjTr = m_pHeadObj->GetTransform();

		m_pShootingObj = m_pHeadObj->FindChildFromTag("pUserShootingPoint");
		m_pShootingObjTr = m_pHeadObj->GetTransform();
	...
	if (startFlag == true)
	{
		...
		CDx11GameObject* pAim = pLayer->FindObjectFromTag("pAimObj");
		CDx11Renderer* pRenderer = (CDx11Renderer*)pAim->FindComponentFromType(CT_RENDERER);

		CDx11GameObject* pMonster = pLayer->FindObjectFromTag("pMonsterObj");
		CDx11FSM* pFSM = (CDx11FSM*)pMonster->FindComponentFromType(CT_FSM);

		pRenderer->SetMaterialDiffuseTex("Aim");
		pFSM->SetState<CWalkState>(pMonster);
...
void CInGameSceneSC::CreateAim()
{
	...
	pRenderer->CreateMaterial("Wait", L"Wait.png");
...
void CInGameSceneSC::CreateMonster()
{
	// 레이어를 찾는다.
	CDx11Layer*	pLayer = m_pScene->FindLayer("DefaultLayer");
...
void CInGameSceneSC::CreateUI()
{
	// 레이어를 찾는다.
	CDx11Layer*	pLayer = m_pScene->FindLayer("DefaultLayer");

	RESOLUTION_TYPE	eRSType = DX11_GET_SINGLE(CDx11Device)->GetResolutionType();
	float OneFifthX = (float)g_tRS[eRSType].iWidth / 5;
	float OneFifthY = (float)g_tRS[eRSType].iHeight / 5;

	// HP Frame
	CDx11GameObject*	pMapUI = CDx11GameObject::Create("pMapUI", OCT_NORMAL);

	// 렌더러를 설정한다.
	CDx11Renderer*	pRenderer = pMapUI->AddComponent<CDx11Renderer>("pMapUIRenderer");
	pRenderer->SetCameraTag("UICamera");
	pRenderer->SetMesh("UIMesh");
	pRenderer->SetShader("UIShader");
	// pRenderer->SetRenderState(DX11_RS_DEPTH_DISABLE, RSF_DEPTHSTENCIL);
	pRenderer->SetRenderState(DX11_RS_ALPHABLEND, RSF_BLEND);
	pRenderer->CreateMaterial("pMapUI", L"map.png");
	SAFE_RELEASE(pRenderer);

	// 크기를 조정한다.
	CDx11Transform*	pTr = pMapUI->GetTransform();
	pTr->SetWorldPos(OneFifthX*3.5, OneFifthY*3, 1.f);
	pTr->Scaling(OneFifthX, OneFifthX, 1.f);
	SAFE_RELEASE(pTr);

	// 레이어에 추가한다.
	pLayer->AddObject(pMapUI);

	CDx11Layer* pPosLayer =  m_pScene->CreateLayer("PosLayer");

	// 몬스터UI 30 마리를 추가한다.
	for (int i = 0; i < 30; ++i)
	{
		CDx11GameObject*	pMonsterUI  = CDx11GameObject::Create("pMonsterUI");
		pPosLayer->AddObject(pMonsterUI);

		// 렌더러를 설정한다.
		CDx11Renderer*	pRenderer = pMonsterUI->AddComponent<CDx11Renderer>("pMonsterUI");
		pRenderer->SetCameraTag("UICamera");
		pRenderer->SetMesh("UIMesh");
		pRenderer->SetShader("UIShader");
		// pRenderer->SetRenderState(DX11_RS_DEPTH_DISABLE, RSF_DEPTHSTENCIL);
		pRenderer->SetRenderState(DX11_RS_ALPHABLEND, RSF_BLEND);
		pRenderer->CreateMaterial("pMonsterUI", L"pos.png");
		SAFE_RELEASE(pRenderer);

		// 크기를 조정한다.
		CDx11Transform*	pTr = pMonsterUI->GetTransform();
		pTr->Scaling(OneFifthX/20, OneFifthX /20, 1.f);
		pTr->SetWorldPos(OneFifthX, OneFifthX, 0);
		SAFE_RELEASE(pTr);

		SAFE_RELEASE(pMonsterUI);
	}

	//// 캐릭터 한마리도 추가한다.
	//CDx11GameObject*	pPlayerUI = CDx11GameObject::Create("pPlayerUI");
	//pPosLayer->AddObject(pPlayerUI);

	//// 렌더러를 설정한다.
	//pRenderer = pPlayerUI->AddComponent<CDx11Renderer>("pPlayerUI");
	//pRenderer->SetCameraTag("UICamera");
	//pRenderer->SetMesh("UIMesh");
	//pRenderer->SetShader("UIShader");
	//// pRenderer->SetRenderState(DX11_RS_DEPTH_DISABLE, RSF_DEPTHSTENCIL);
	//pRenderer->SetRenderState(DX11_RS_ALPHABLEND, RSF_BLEND);
	//pRenderer->CreateMaterial("pPlayerUI", L"pos2.png");
	//SAFE_RELEASE(pRenderer);

	//// 크기를 조정한다.
	//pTr = pPlayerUI->GetTransform();
	//pTr->Scaling(OneFifthX / 20, OneFifthX / 20, 1.f);
	//pTr->SetWorldPos(OneFifthX, OneFifthX, 0);
	//SAFE_RELEASE(pTr);
	...
	SAFE_RELEASE(pPosLayer);
	SAFE_RELEASE(pMapUI);
	SAFE_RELEASE(pLayer);
}

void CInGameSceneSC::UpdateUI()
{
	// 몬스터들을 찾는다.
	// 그러려면 우선 레이어를 찾고,
	// 레이어 안에 몬스터를 찾는다.
	...
	// 맵에 위치를 찍는 레이어를 찾는다.
	CDx11Layer*	pPosLayer = m_pScene->FindLayer("PosLayer");
	list<CDx11GameObject*>* pPoints = pPosLayer->FindObjectsFromTag("pMonsterUI");

	// 리스트를 돌면서 몬스터의 위치에 점을 찍는다.
	list<CDx11GameObject*>::iterator	iterPos;
	list<CDx11GameObject*>::iterator	iterPosEnd = pPoints->end();

	// 이 리스트를 돌면선 초기 위치를 설정한다.
	for (iterPos = pPoints->begin(); iterPos != iterPosEnd; ++iterPos)
	{
		CDx11Transform* pTr = (*iterPos)->GetTransform();
		pTr->SetWorldPos(-200, -200, 0);
		SAFE_RELEASE(pTr);
	}



	iterPos = pPoints->begin();
	...
	list<CDx11GameObject*>* pMonsters = pLayer->FindObjectsFromTag("pMonsterObj");

	// 리스트를 돌면서 몬스터의 위치에 점을 찍는다.
	list<CDx11GameObject*>::iterator	iter;
	list<CDx11GameObject*>::iterator	iterEnd = pMonsters->end();

	for (iter = pMonsters->begin(); iter != iterEnd; ++iter)
	{
		CDx11Transform* pTr =  (*iter)->GetTransform();
		Vec3 Pos =  pTr->GetWorldPos();

		float percentX = Pos.x / 200;
		float percentY = Pos.z / 200;

		// 내 점의 위치는 시작위치 + (길이*비율)
		float myPosX = OneFifthX*3.5 + (OneFifthX * percentX);
		float myPosY = OneFifthY*3 + (OneFifthX * (1-percentY));

		// 화면에 그린다.
		CDx11Transform* pPos =  (*iterPos)->GetTransform();
		pPos->SetWorldPos(myPosX, myPosY, 0);
		SAFE_RELEASE(pPos);
	...
	//CDx11GameObject* pTank =  pLayer->FindObjectFromTag("pTankObj");
	//CDx11GameObject* pTankBody = pTank->FindChildFromTag("pBodyObj");

	//SAFE_RELEASE(pTankBody);
	//SAFE_RELEASE(pTank);
...
void CInGameSceneSC::CreateTank(BYTE * pPacket)
{
	...
	pTr->SetWorldPos(vPos.x, vPos.y, vPos.z);
	SAFE_RELEASE(pTr);
...
void CInGameSceneSC::CreateMonster(BYTE * pPacket)
{
	//typedef pair<Vec3, Vec3>	monsterInfo;
	//vector<monsterInfo> m_vMonsterInfo;
	////	list<monsterInfo> m_MonsterInfo;
	...
	//	m_vMonsterInfo.push_back(make_pair(vPos, vRot));
	//	//	m_MonsterInfo.push_back(make_pair(vPos, vRot));
	//}
...
//	{
		...
		CDx11Transform* pTr = pMonsterObj->GetTransform();
		pTr->SetWorldPos(vPos.x, vPos.y, vPos.z);
		pTr->SetWorldRot(vRot.x, vRot.y, vRot.z);
//		pTr->SetWorldPos(p->first.x, p->first.y, p->first.z);
//		pTr->SetWorldRotY(DX11_PI / 180 * p->second.y);
...
void CInGameSceneSC::MoveOtherUser(BYTE * pPacket)
{
	if (!m_pUserPlayer)	return;
...
void CInGameSceneSC::RotateOtherUser(BYTE * pPacket)
{
	if (!m_pUserPlayer)   return;
	...
	CDx11Transform* pTr = m_pBodyObj->GetTransform();
	Vec3      vAxisZ = pTr->GetWorldAxis(AXIS_X);
	vAxisZ.y = 0;

	// 받아온 축과 내 축을 내적한다.
	float fAngle = vRot.Dot(vAxisZ);

	// Y축을 기준으로 회전한다.
	Vec3      vAxisY = pTr->GetWorldAxis(AXIS_Y);
	pTr->RotateWorld(fAngle, 1, vAxisY);

	SAFE_RELEASE(pTr);
}

void CInGameSceneSC::RotateHeadOtherUser(BYTE * pPacket)
{
	if (!m_pBodyObj)	return;

	CZEROStreamSP	stream;
	Vec3	vAxisZ;

	stream->SetBuffer(pPacket);
	stream->ReadFloat(&vAxisZ.x);
	stream->ReadFloat(&vAxisZ.y);
	stream->ReadFloat(&vAxisZ.z);

	vAxisZ.y = 0;

	Vec3		myHeadAxisZ = m_pHeadObjTr->GetWorldAxis(AXIS_Z);
	myHeadAxisZ.y = 0;

	// 받아온 축과 내 축을 내적한다.
	float fAngle = vAxisZ.Dot(myHeadAxisZ);

	// Y축을 기준으로 회전한다.
	Vec3 vMyHeadAxisY = m_pHeadObjTr->GetWorldAxis(AXIS_Y);
	m_pHeadObjTr->RotateWorld(fAngle, 1, vMyHeadAxisY);
}

void CInGameSceneSC::ShootOtherUser(BYTE * pPacket)
{
	// 레이어를 찾는다.
	CDx11Layer*	pLayer = m_pScene->FindLayer("DefaultLayer");
	...
	// 총알 오브젝트 추가
	CDx11GameObject*	pBulletObj = CDx11GameObject::Create("pUserBulletObj");
	pLayer->AddObject(pBulletObj);

	// 초기 위치를 정한다.
	CDx11Transform*		pBulletObjTr = pBulletObj->GetTransform();
	pBulletObjTr->Copy(m_pShootingObjTr);
	pBulletObjTr->Scaling(0.05, 0.05, 0.05);
	pBulletObjTr->SetWorldView(vDir);

	// 렌더러를 생성한다.
	CDx11Renderer* pRenderer = pBulletObj->AddComponent<CDx11Renderer>("pUserBulletObjRenderer");
	pRenderer->SetMesh("ColorSphere");
	pRenderer->SetShader("DefaultShader");
	pRenderer->SetMaterialDiffuseTex("Default");

	// 총알 스크립트를 컴포넌트에 추가한다.
	CBulletSC*	pBulletSC = pBulletObj->AddComponent<CBulletSC>("pUserBulletSC");
...
void CInGameSceneSC::ChangeScene()
{
	// 인게임 씬을 생성한다.
	CDx11Scene*   pScene = DX11_GET_SINGLE(CDx11SceneManager)->CreateScene("LoadingScene");
	CEndSceneSc* pMainSceneSC = pScene->AddScript<CEndSceneSc>();
