...
void CDx11Transform::ChildRot(const Vec3& rot)
{
	...
	XMVECTOR V = XMVectorSet(rot.x, rot.y, rot.z, FLOAT(1.0));
...
void CDx11Transform::SetPrePos()
{
	SetWorldPos (m_vPrePos);
}
...
void CDx11Transform::SetWorldView(AXIS axis)
{
	m_vView.x = m_vAxis[axis].x;
	m_vView.y = m_vAxis[axis].y;
	m_vView.z = m_vAxis[axis].z;
}
...
void CDx11Transform::RotateWorld(float fAngle, float fTime, const Vec3 & axis)
{
	Vec3 vRot = axis * fAngle * fTime;
...
void CDx11Transform::SetWorldPos(float fTime, const Vec3& P)
{
	Vec3	vPos(
		P.x - XMVectorGetX(m_T),
		P.y - XMVectorGetY(m_T),
...
void CDx11Transform::RotateAround(XMFLOAT3 point, XMFLOAT3 axis, FLOAT angle)
{
